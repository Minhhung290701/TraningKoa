...
router.get('/', async (ctx, next) => {
    await knex.select('*').from('users').then((rows)=>{
        ctx.body = rows;
    })
    next();
})
...
router.get('/:id', async (ctx, next) => {
    let check = false;
    await knex.select("*").from('users').where('id',ctx.params.id).then((rows) => {
        ctx.body = rows.length > 0 ? rows : "Khong tim thay user";
    });
})
...
router.post('/', async (ctx,next) => {
    let maxid = 0;
    await knex.select('id').from('users').then((rows) => {
        for(x of rows) {
